...
exports.default = new Interaction_1.default('limit', async (client, modal, config, res) => {
    ...
    if (0 > count || isNaN(count)) {
        return modal.editReply({
            embeds: [new EmbedBuilder_1.default().default(modal.member, config.buttons[modal.customId].title, i18n_1.i18n.t("modals.limit.slots_number_must_be_positive"))]
        });
    }
    if (res) {
        res.limit = count;
    ...
    return modal.editReply({
        embeds: [new EmbedBuilder_1.default().default(modal.member, config.buttons[modal.customId].title, i18n_1.i18n.t("modals.limit.slots_number_change_success", { voice_name: modal.member.voice.channel.toString() }))]
    });
});
